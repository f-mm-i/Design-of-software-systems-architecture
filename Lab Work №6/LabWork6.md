...
**Проблема, которую затрагивает:**  
Системе нужен объект, который будет принимать внешние запросы и координировать выполнение серверной логики, не смешивая HTTP-обработку с низкоуровневой работой с БД.

**Решение, которое позволяет справиться с проблемой:**  
В проекте *Mental Maps* роль Controller выполняют маршруты `maps.js` и `reports.js` (или концептуально классы `MapsController` и `ReportsController`).  
Они принимают HTTP-запрос, извлекают параметры, вызывают прикладную логику и возвращают ответ.
...
router.get("/:mapId", requireAuth, async (req, res, next) => {
  try {
    ...
    const response = await mapsFacade.getDetailedMap(mapId);
...
- логика взаимодействия с клиентом отделяется от бизнес-логики;
- код становится понятнее и легче поддерживается;
- контроллеры можно комбинировать с фасадами, сервисами и прокси.

**Связь с другими паттернами:**  
Связан с **Facade**, **Proxy**, **Command**.
...
**Проблема, которую затрагивает:**  
Нужно определить, какой объект должен отвечать за создание других объектов системы, чтобы не размазывать создание сущностей по маршрутам и контроллерам.

**Решение, которое позволяет справиться с проблемой:**  
В проекте *Mental Maps* роль Creator выполняют фабрики элементов карты (`PointElementFactory`, `NoteElementFactory`, `RouteElementFactory`).  
Именно они создают объекты элементов, так как содержат знания о нужных полях и о способе инициализации конкретного типа элемента.
...
**Результаты, которые при этом достигаются:**  
- создание объектов централизуется;
- уменьшается дублирование логики;
- добавление новых типов элементов происходит без изменения кода маршрута.

**Связь с другими паттернами:**  
Связан с **Factory Method**, **Polymorphism**.
...
**Проблема, которую затрагивает:**  
Нужно определить, какой объект должен принимать решение, если это решение зависит от имеющихся у него данных.

**Решение, которое позволяет справиться с проблемой:**  
В проекте *Mental Maps* роль Information Expert выполняет `AuthorizedMapsProxy`, так как именно он обладает необходимой информацией для решения, может ли пользователь читать карту:  
- видимость карты (`public/private`);
- владелец карты;
...
**Результаты, которые при этом достигаются:**  
- решения принимаются там, где сосредоточена нужная информация;
- уменьшается количество дублирующихся проверок в маршрутах;
- проще изменять правила доступа.

**Связь с другими паттернами:**  
Связан с **Proxy**, **Facade**.
...
**Проблема, которую затрагивает:**  
Иногда для улучшения архитектуры нужен искусственно выделенный класс, который не является доменной сущностью, но помогает снизить связанность и повысить связность кода.

**Решение, которое позволяет справиться с проблемой:**  
В проекте *Mental Maps* роль Pure Fabrication выполняют адаптеры ответов (`MapResponseAdapter`, `ElementResponseAdapter`, `ReportResponseAdapter`).  
Они не являются сущностями предметной области, но специально созданы для преобразования данных БД в удобный формат API-ответа.
...
**Результаты, которые при этом достигаются:**  
- маршруты не перегружаются преобразованием данных;
- формат БД и формат API разделяются;
- код становится чище и проще в сопровождении.

**Связь с другими паттернами:**  
Связан с **Adapter**, **Low Coupling**, **High Cohesion**.
...
**Проблема, которую затрагивает:**  
Прямая связь между компонентами может привести к излишней зависимости и затруднить изменение архитектуры.

**Решение, которое позволяет справиться с проблемой:**  
В проекте *Mental Maps* роль Indirection выполняет `MapsFacade`, который становится промежуточным звеном между маршрутом и подсистемами доступа к данным/адаптации ответа.  
Маршрут обращается не напрямую к БД и адаптерам, а к фасаду, который координирует работу внутренних компонентов.
...
**Результаты, которые при этом достигаются:**  
- маршруты зависят от меньшего числа компонентов;
- изменения внутри подсистемы меньше влияют на внешний код;
- архитектура становится гибче.

**Связь с другими паттернами:**  
Связан с **Facade**, **Proxy**, **Protected Variations**.
...
**Проблема, которую затрагивает:**  
Слишком сильная связанность классов приводит к тому, что изменение одного компонента вызывает каскад изменений в других.

**Решение, которое позволяет справиться с проблемой:**  
В проекте *Mental Maps* низкая связанность достигается за счёт того, что маршруты обращаются не напрямую к БД, а к фасадам, прокси, адаптерам и сервисам.  
Каждый класс знает только о ближайшем интерфейсе взаимодействия, а не о деталях всей системы.
...
**Результаты, которые при этом достигаются:**  
- систему проще менять и расширять;
- меньше риск сломать соседние компоненты;
- код легче тестировать.

**Связь с другими паттернами:**  
Связан с **Facade**, **Adapter**, **Proxy**, **Pure Fabrication**.
...
**Проблема, которую затрагивает:**  
Если класс отвечает сразу за слишком много задач, он становится трудно читаемым, трудно тестируемым и плохо расширяемым.

**Решение, которое позволяет справиться с проблемой:**  
В проекте *Mental Maps* высокая связность обеспечивается разделением ответственности:
- `ResponseAdapters` отвечают только за преобразование данных;
- `MapsFacade` — за координацию работы с картой;
- `AuthorizedMapsProxy` — за контроль доступа;
- `LoggingMapsServiceDecorator` — за логирование.
...
**Результаты, которые при этом достигаются:**  
- каждый класс имеет узкую и понятную ответственность;
- код легче поддерживать;
- изменения локализуются в пределах одного класса.

**Связь с другими паттернами:**  
Связан с **Decorator**, **Adapter**, **Facade**.
...
**Проблема, которую затрагивает:**  
Если поведение системы зависит от типа объекта, код быстро переполняется условиями `if/switch`.

**Решение, которое позволяет справиться с проблемой:**  
В проекте *Mental Maps* полиморфизм используется в стратегиях валидации элементов карты и в фабриках элементов.  
Вместо большого количества условных операторов система выбирает объект нужного типа и вызывает единый метод (`validate` или `createElement`).
...
- код становится легче расширять;
- новые типы можно добавлять без переписывания основной логики.

**Связь с другими паттернами:**  
Связан с **Strategy**, **Factory Method**, **Creator**.
...
**Проблема, которую затрагивает:**  
Система должна быть устойчивой к изменениям: изменению формата данных, правил доступа, состава ответа, типов элементов карты.

**Решение, которое позволяет справиться с проблемой:**  
В проекте *Mental Maps* защищённость от изменений достигается через выделение промежуточных абстракций:
- адаптеры скрывают формат БД;
- фасад скрывает сложность работы с картой;
- прокси скрывает и централизует правила доступа;
- стратегии скрывают различия валидации;
- фабрики скрывают детали создания объектов.

**Пример фрагмента программного кода:**  
```js
const strategy = resolveValidationStrategy(req.body.type);
const context = new ElementValidationContext(strategy);
const details = context.validate(req.body);
```
**Результаты, которые при этом достигаются:**  
- локализация изменений в конкретных классах;
- уменьшение количества мест, которые нужно менять при развитии проекта;
- улучшение сопровождаемости и устойчивости архитектуры.

**Связь с другими паттернами:**  
Связан с **Facade**, **Proxy**, **Adapter**, **Strategy**, **Factory Method**.
