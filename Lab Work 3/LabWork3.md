...
*(Изображение диаграммы контейнеров вставляется отдельно.)*
...
*(Изображение диаграммы компонентов Mobile App вставляется отдельно.)*
...
*(Изображение диаграммы компонентов Backend вставляется отдельно.)*
...
*(Изображение диаграммы последовательностей вставляется отдельно.)*
...
*(Изображение UML-диаграммы классов вставляется отдельно.)*

---

## 6. Принципы KISS, YAGNI, DRY, SOLID — как учтены в реализации (пояснение без кода)

> Код клиентской и серверной части будет добавлен отдельно. Здесь — пояснение, какие решения приняты, чтобы соответствовать принципам.
...
- выбран и описан вариант использования UC-03;
- подготовлены (для вставки) диаграммы C4 контейнеров и компонентов;
- подготовлена (для вставки) диаграмма последовательностей, показывающая взаимодействие компонентов;
- подготовлена (для вставки) UML-диаграмма классов, описывающая модель данных (>= 5 сущностей);
- описано, как в реализации учитываются KISS, YAGNI, DRY, SOLID;
- рассмотрены принципы BDUF, SoC, MVP, PoC с обоснованием применимости/отказа.
